# this tools are copied from this github repo:
# https://github.com/satyenrajpal/speed-prediction-challenge/blob/master/utils.py
...
def plot(predict_values, gt):
    fig, ax = plt.subplots()
    ax.plot(np.arange(len(gt)), gt, label='Exact Speed')
    ax.plot(np.arange(len(predict_values)), np.array(predict_values), label='Optical Flow prediction')
